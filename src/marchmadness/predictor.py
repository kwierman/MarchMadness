...
class MarchMadnessPredictor:
    ...
    def __init__(self, season: int = 2025, cache_dir: str = ".mm_cache",
                 verbose: bool = True):
        self.season    = season
    ...
    def load_data(self, offline: bool = False) -> "MarchMadnessPredictor":
        ...
        if self.verbose:
            print(f"[Predictor] Loaded {len(self.teams_df)} teams, "
    ...
    def report(
        ...
        if sim_results is None:
            raise RuntimeError("No simulation results. Run run_tournament_simulation().")
    ...
    def _print_summary(self, sim_results: dict, top_n: int = 16) -> None:
        ...
        for rank, team in enumerate(champ_probs.head(top_n).index, 1):
            ...
            print(f"  {rank:<4} {team:<22} {seed!s:<6} {conf:<10} "
