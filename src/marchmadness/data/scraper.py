...
class DataScraper:
    ...
    def __init__(self, season: int = 2025, cache_dir: str = ".mm_cache", use_cache: bool = True):
        self.season    = season
    ...
    def fetch_all(self, offline: bool = False) -> dict[str, pd.DataFrame]:
        ...
        try:
            ...
            print(f"[DataScraper] Scrape error ({exc}) — falling back to synthetic data.")
            return self._synthetic_data()

        return {"teams": self.teams_df, "games": self.games_df, "players": self.players_df}

    # ── real scrapers ─────────────────────────────────────────────────────

    def _fetch_team_stats(self) -> pd.DataFrame:
        """Scrape per-100-possession team stats from sports-reference.com."""
        url = f"{SPORTS_REF_BASE}/seasons/men/{self.season}-school-stats.html"
        ...
        df = df[df["School"].notna() & ~df["School"].str.contains("School|Conf", na=True)]
    ...
    def _fetch_game_results(self) -> pd.DataFrame:
        ...
        url = f"{SPORTS_REF_BASE}/seasons/men/{self.season}-schedule.html"
    ...
    def _fetch_player_stats(self) -> pd.DataFrame:
        ...
        url = f"{SPORTS_REF_BASE}/seasons/men/{self.season}-players.html"
    ...
    def _synthetic_data(self) -> dict[str, pd.DataFrame]:
        ...
        for seed, team, conf, region in tournament_teams:
            ...
            def_rtg  = rng.normal(98  - (16 - seed) * 0.8, 2.5)  # defensive rating (lower=better)
            ...
            teams.append({
                "team": team, "seed": seed, "conference": conf, "region": region,
        ...
        for _ in range(1200):
            ...
            games.append({"team1": t1, "team2": t2, "winner": winner,
                          "loser": loser, "margin": margin})
        ...
        for _, row in teams_df.iterrows():
            ...
            for p in range(n_players):
                ...
                players.append({
                    ...
                    "team": row["team"],
                    "ppg": round(rng.normal(row["ppg"] / 5 * role_weight * 1.5, 2), 1),
                    "rpg": round(rng.normal(row["rpg"] / 5 * role_weight, 1), 1),
                    "apg": round(rng.normal(row["apg"] / 5 * role_weight, 0.8), 1),
                    "fg_pct": round(rng.normal(0.46 + row["seed"] * -0.003, 0.04), 3),
                    "three_pt_pct": round(rng.normal(0.35, 0.05), 3),
                    "per": round(rng.normal(18 - row["seed"] * 0.3 + role_weight * 5, 3), 1),
